...
def exam2(request):
    ...
    # http://127.0.0.1:8000/secondapp/exam2/?name=하재연(utf-8) : ? 뒤에 문자열 = 쿼리 || request.GET.get('name', 'JHA') : dictionary.get() 과 같음
    context = {'result': name}
...
def exam2_1(request):
    ...
    if request.method == 'GET':
        msg = "GET 방식으로 요청했군요...ㅎ"
    else :
        msg = "POST 방식으로 요청했군요...ㅎ"
    context = {'result': msg}
    return HttpResponse(template.render(context, request))
...
def exam2_2(request):
    if request.method == 'GET':
        msg = request.GET.get("info1", "없음") + "-" + request.GET.get("info2", "없음") + "-" +\
              request.GET.get("info3", "없음")
    else:
        msg = request.POST.get("info1", "없음") + "-" + request.POST.get("info2", "없음") + "-" +\
              request.POST.get("info3", "없음")
    context = {'result': msg}
    return render(request, 'exam2_2.html', context)
